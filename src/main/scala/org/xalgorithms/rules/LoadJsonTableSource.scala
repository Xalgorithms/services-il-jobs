...
abstract class LoadJsonTableSource extends LoadTableSource {
  ...
  def flatten(v: JsValue): Map[String, Value] = v match {
    case (o: JsObject) => o.fields.foldLeft(Map[String, Value]()) { (m, tup) =>
      m ++ make_value(tup._1, tup._2)
    }
    case _ => Map()
  }

  def make_table(v: JsValue): Seq[Map[String, Value]] = v match {
    case (a: JsArray)  => a.value.map(flatten)
    case _ => Seq()
  }

  def make_value(k: String, v: JsValue): Map[String, Value] = v match {
